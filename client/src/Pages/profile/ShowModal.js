...
const ShowModal = ({isShowModalOpen, setIsShowModalOpen, selectedTheatre}) => {
          ...
          render: (text, data) => {
            return(
              <div className='d-flex align-items-center gap-10'>
                <Button onClick={() => { setView("edit"); setSelectedMovie(data.movie); setSelectedShow({...data, date: moment(data.date).format("YYYY-MM-DD")}); console.log(selectedMovie && selectedMovie.title) }} ><EditOutlined/></Button>
    ...
    return(
        ...
        {(view === "form" || view === "edit") && <Form className='' layout='vertical' style={{width: "100%"}} initialValues={view === "edit" ? selectedShow : null}  onFinish={onFinish}>
                <Row gutter={{
                    xs: 6,
                    sm: 10,
                    md: 12,
                ...
                <Col span={24}>
                    <Row  gutter={{
                        ...
                        <Col span={8}>
                            <Form.Item label="Show Name" htmlFor='name' name="name" className='d-block' rules={[{required: true, message: "Show name is required!"}]}>
                                <Input id="name" type="text" placeholder='Enter the show name'></Input>
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                            <Form.Item label="Show Date" htmlFor='date' name="date" className='d-block' rules={[{required: true, message: "Show date is required!"}]}>
                                    <Input id="date" type="date" placeholder='Enter the show date'></Input>                                
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                            <Form.Item label="Show Timing" htmlFor='time' name="time" className='d-block' rules={[{required: true, message: "Show time is required!"}]}>
                                    <Input id="time" type="time" placeholder='Enter the show date'></Input>                                
                            </Form.Item>
                        </Col>
                    </Row>
                </Col>
                <Col span={24}>
                    <Row  gutter={{
                        ...
                        <Col span={8}>
                            <Form.Item  label="Select the Movie" htmlFor='movie' name="movie" className='d-block' rules={[{required: true, message: "Movie  is required!"}]}>
                            <Select id="movie" placeholder='Select Movie' defaultValue={selectedMovie && selectedMovie.title} style={{ width: "100%", height: "45px" }} options={
                                movies && movies.map(movie => ({key: movie._id, value: movie._id, label: movie.title}))
                            } />
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                            <Form.Item  label="Ticket Price" htmlFor='ticketPrice' name="ticketPrice" className='d-block' rules={[{required: true, message: "Ticket price is required!"}]}>
                                <Input id="ticketPrice" type="number" placeholder='Enter the ticket price'></Input>
                            </Form.Item>                
                        </Col>
                        <Col span={8}>
                            <Form.Item  label="Total Seats" htmlFor='totalSeats' name="totalSeats" className='d-block' rules={[{required: true, message: "Total seats are required!"}]}>
                                <Input id="totalSeats" type="number" placeholder='Enter the number of total seats'></Input>
                            </Form.Item>                
                        </Col>
                    </Row>
                </Col>          
